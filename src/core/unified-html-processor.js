 ...
 */
export async function processHtmlUnified(htmlContent, filePath, sourceRoot, dependencyTracker, config = {}) {
  ...
  try {
    ...
    logger.debug(`Processing SSI includes for: ${path.relative(sourceRoot, filePath)}`);
    ...
    if (hasDOMTemplating(processedContent)) {
      logger.debug(`Processing DOM templating for: ${path.relative(sourceRoot, filePath)}`);
    ...
    logger.error(`Unified HTML processing failed for ${path.relative(sourceRoot, filePath)}: ${error.message}`);
...
function hasDOMTemplating(content) {
  ...
  return content.includes('extends=') || 
...
async function processDOMTemplating(htmlContent, filePath, sourceRoot, config) {
  try {
    ...
    if (templateElement) {
      ...
      return await processTemplateExtends(htmlContent, extendsPath, filePath, sourceRoot, config);
    }
    ...
    if (layoutElement) {
      ...
      return await processLayoutAttribute(htmlContent, layoutAttr, filePath, sourceRoot, config);
    }
    ...
    throw new ComponentError(filePath, `DOM templating processing failed: ${error.message}`, filePath);
  }
}
 ...
 */
async function processTemplateExtends(pageContent, templatePath, filePath, sourceRoot, config) {
  ...
  if (templatePath.startsWith('/')) {
    ...
    resolvedTemplatePath = path.join(sourceRoot, config.layoutsDir, templatePath);
  }
 ...
 */
async function processLayoutAttribute(pageContent, layoutPath, filePath, sourceRoot, config) {
  // Similar to template extends but for data-layout attribute
  return await processTemplateExtends(pageContent, layoutPath, filePath, sourceRoot, config);
}
...
function extractSlotData(htmlContent) {
  ...
  const slottedElements = document.querySelectorAll('[slot], template[data-slot]');
  slottedElements.forEach(element => {
    const slotName = element.getAttribute('slot') || element.getAttribute('data-slot');
  ...
  if (layoutElement) {
    ...
    const templateElements = layoutElement.querySelectorAll('template[data-slot]');
...
function applySlots(templateContent, slotData) {
  ...
  for (const [slotName, content] of Object.entries(slotData)) {
    const slotRegex = new RegExp(`<slot\\s+name=["']${slotName}["'][^>]*>.*?</slot>`, 'gs');
    const selfClosingSlotRegex = new RegExp(`<slot\\s+name=["']${slotName}["'][^>]*/>`, 'g');
...
function processStandaloneSlots(htmlContent) {
  ...
  return htmlContent.replace(/<slot[^>]*><\/slot>/gs, '')
                   .replace(/<slot[^>]*\/>/g, '');
}
